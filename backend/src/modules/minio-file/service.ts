...
class MinioFileProviderService extends AbstractFileProviderService {
  ...
  constructor(
    ...
    this.logger_.info(
      `MinIO service initialized with bucket: ${this.bucket}, prefix: ${this.config_.prefix || "none"}`
    );
  ...
  async upload(file: ProviderUploadFileDTO): Promise<ProviderFileResultDTO> {
    ...
    try {
      ...
      this.logger_.info(
        `Successfully uploaded file ${fileKey} to MinIO bucket ${this.bucket}`
      );
  ...
  async delete(
    ...
    for (const file of files) {
      ...
      try {
        ...
        this.logger_.info(
          `Successfully deleted file ${file.fileKey} from MinIO bucket ${this.bucket}`
        );
      } catch (error) {
        this.logger_.warn(
  ...
  async getDownloadStream(fileData: ProviderGetFileDTO): Promise<Readable> {
    ...
    try {
      ...
      const minioStream = await this.client.getObject(
        this.bucket,
        fileData.fileKey
      );

      this.logger_.info(
        `Retrieved download stream for file ${fileData.fileKey}`
      );
      return minioStream;
    } catch (error) {
