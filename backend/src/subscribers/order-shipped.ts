...
export default async function orderShippedHandler({
  ...
  const fulfillment = await (fulfillmentModuleService as any).retrieveFulfillment(
    fulfillmentId,
    { relations: ['labels'] }
  )
  ...
  if (!orderId) {
    console.error('[order-shipped] Could not find order for fulfillment', fulfillmentId)
    return
  }
